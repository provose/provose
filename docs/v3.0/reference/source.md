...
When using Provose, you should set `source` to be `"github.com/provose/provose?ref=v3.0.0-beta1"`. Always make sure that the `ref=` parameter is pinned to a specific version to avoid Provose upgrading without your intention.
...
```terraform
{% include v3.0/reference/source/source.tf %}
```
